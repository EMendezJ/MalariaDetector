...
%% Método de Euler para el Cálculo de la rapidez angular y ángulo como
función del tiempo
t = 0;
T(1) = 0; Vx(1) = 0; Vy(1)=0; X(1) = -d1*cos(theta0); Y(1) =
H-d1*sin(theta0);
omegaV(1) = omega0;
thetaV(1) = theta0;
for k=2:100000
...
%fprintf("Sin fricción: tv = %f, angSal(Deg) = %f, R(cm) = %f
...
plot([-d1*cos(thetaV(i)),d2*cos(thetaV(i))],[H-d1*sin(thetaV(i)),H+d2*sin(the
taV(i))],'- k','LineWidth',2);
end
pause(0.00000001);
